(function () {
  ...
  var PLACEHOLDER =
    '<div class="w-full h-28 bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center">' +
  ...
  var Views = {
    card: function (item, section) {
      ...
      if (item.browserId && item.browserId === Utils.getBrowserId()) {
        deleteBtn =
          '<button data-delete data-id="' + item.id + '" title="حذف" ' +
          'class="absolute top-1.5 right-1.5 w-6 h-6 bg-white rounded-full shadow flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition">' +
          TRASH_SVG +
          "</button>";
      }

      return (
        '<article data-id="' + item.id + '" class="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-lg cursor-pointer transition">' +
        '<div class="relative">' + img + deleteBtn + "</div>" +
        '<div class="p-2 space-y-1">' +
        '<h3 class="text-sm font-semibold text-slate-900 truncate">' + Utils.esc(item.name) + "</h3>" +
    ...
    renderList: function (el, emptyEl, items, section) {
      ...
      el.innerHTML = items
        .map(function (item) {
          return Views.card(item, section);
        })
        .join("");
    },
    ...
    fillTypes: function (select, types) {
      ...
      var html = '<option value="all">كل الأنواع</option>';
